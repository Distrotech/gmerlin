...
static int read_line_utf16(bgav_input_context_t * ctx,
  ...
  *ret_len = 0;
  ...
  while(1)
    {
    if(!bgav_input_read_16_le(ctx, &c))
      {
      eof = 1;
      break;
      }
...
static int read_subtitle_srt(bgav_stream_t * s)
  ...
  if(priv->read_char_16)
    ...
    while(1)
      {
      bgav_input_read_line(ctx->input, &(priv->buf_8), &(priv->buf_8_alloc), 0);
      ...
      fprintf(stderr, "Got subtitle line: %s\n", priv->buf_8);
...
void bgav_subtitle_reader_close(bgav_subtitle_reader_context_t * ctx)
  {
