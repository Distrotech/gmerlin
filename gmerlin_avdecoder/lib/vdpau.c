...
bgav_vdpau_context_create_video_surface(bgav_vdpau_context_t * ctx,
  ...
  {
  VdpVideoSurface ret;
  if(ctx->video_surface_create(ctx->device,
                               chroma_type, width, height, &ret) != VDP_STATUS_OK)
    {
...
bgav_vdpau_context_create_decoder(bgav_vdpau_context_t * ctx,
  ...
  {
  VdpDecoder ret;
  if(ctx->decoder_create(ctx->device, profile,
                         width, height, max_references, &ret) != VDP_STATUS_OK)
    {
    return VDP_INVALID_HANDLE;
    }
  return ret;
...
bgav_vdpau_context_decoder_render(bgav_vdpau_context_t * ctx,
  ...
  {
  return ctx->decoder_render(dec, target, picture_info, bitstream_buffer_count,
                             bitstream_buffers);
  }
...
void bgav_vdpau_context_surface_to_frame(bgav_vdpau_context_t * ctx,
  ...
  pitches[1] = dst->strides[1];
  pitches[2] = dst->strides[2];
  ctx->video_surface_get_bits_ycbcr(s,
                                    // VDP_YCBCR_FORMAT_YUYV,
                                    VDP_YCBCR_FORMAT_YV12,
                                    (void *const *)(dst->planes), pitches);
  }
