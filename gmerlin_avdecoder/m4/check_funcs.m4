...
AC_DEFUN([GMERLIN_CHECK_LIBVPX],[

AH_TEMPLATE([HAVE_LIBVPX], [Enable libvpx])
have_libvpx="false"

AC_ARG_ENABLE(libvpx,
[AC_HELP_STRING([--disable-libvpx],[Disable libvpx (default: autodetect)])],
[case "${enableval}" in
   yes) test_libvpx=true ;;
   no)  test_libvpx=false ;;
esac],[test_libvpx=true])

if test x$test_libvpx = xtrue; then

OLD_CFLAGS=$CFLAGS
OLD_LIBS=$LIBS
   
LIBS="-lvpx -lpthread -lm"
CFLAGS=""
...
AC_SUBST(LIBVPX_REQUIRED)

AM_CONDITIONAL(HAVE_LIBVPX, test x$have_libvpx = xtrue)

if test x$have_libvpx = xtrue; then
AC_DEFINE(HAVE_LIBVPX)
fi
