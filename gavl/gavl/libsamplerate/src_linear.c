...
static int linear_process_f (SRC_PRIVATE *psrc, SRC_DATA *data) ;
static int linear_process_d (SRC_PRIVATE *psrc, SRC_DATA *data) ;
...
static int
linear_process_f (SRC_PRIVATE *psrc, SRC_DATA *data)
  {	LINEAR_DATA *linear ;
  ...
  while (input_index < 1.0 && linear->out_gen < linear->out_count)
    ...
    for (ch = 0 ; ch < linear->channels ; ch++)
      {	data->data_out_f [linear->out_gen] = linear->last_value_f [ch] + input_index *
  ...
  while (linear->out_gen < linear->out_count && linear->in_used + linear->channels * input_index <= linear->in_count)
    ...
    for (ch = 0 ; ch < linear->channels ; ch++)
      {	data->data_out_f [linear->out_gen] = data->data_in_f [linear->in_used - linear->channels + ch] + input_index *
...
static int
linear_process_d (SRC_PRIVATE *psrc, SRC_DATA *data)
  {	LINEAR_DATA *linear ;
  double		src_ratio, input_index ;
  int			ch ;
  ...
  while (input_index < 1.0 && linear->out_gen < linear->out_count)
    ...
    for (ch = 0 ; ch < linear->channels ; ch++)
      {	data->data_out_d [linear->out_gen] = linear->last_value_d [ch] + input_index *
  ...
  while (linear->out_gen < linear->out_count && linear->in_used + linear->channels * input_index <= linear->in_count)
    ...
    for (ch = 0 ; ch < linear->channels ; ch++)
      {	data->data_out_d [linear->out_gen] = data->data_in_d [linear->in_used - linear->channels + ch] + input_index *
...
const char*
gavl_linear_get_name (int src_enum)
  {
...
const char*
gavl_linear_get_description (int src_enum)
  {
  if (src_enum == SRC_LINEAR)
...
gavl_linear_set_converter (SRC_PRIVATE *psrc, int src_enum, int d)
  ...
  if(d)
    psrc->process = linear_process_d ;
  else
    psrc->process = linear_process_f ;
